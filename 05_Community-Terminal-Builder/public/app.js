...
function bindNftPhaseDateAutofill(){
  ...
  const syncDate=event=>{const input=event.target;if(!(input instanceof HTMLInputElement)||input.dataset.phaseField!=="startDate")return;const card=input.closest(".nft-phase-card"),end=card?.querySelector('[data-phase-field="endDate"]');if(input.value&&end&&!end.value)end.value=input.value;confirmedMintSignature="";syncNftMintSchedule()};
...
async function payload(){const mint=syncNftMintSchedule();return {
  ...
  nft:{openSeaSlug:val("openSeaSlug"),collectionName:val("nftCollectionName"),supply:val("nftSupply"),mode:mint.mode||nftMintMode(),mintAt:mint.ok&&!mint.disabled?mint.iso:"",mintPhases:mint.ok&&!mint.disabled&&mint.mode==="multiple"?mint.phases.map(({id,label,name,startsAt,endsAt,price,limit,timezone})=>({id,label,name,startsAt,endsAt,price,limit,timezone})):[],timezone:mint.timeZone||val("nftMintTimezone")||browserTimeZone()},
...
function configurationReady(){const mint=syncNftMintSchedule();return Boolean(val("projectName")&&val("ticker")&&isEvmAddress(val("tokenContract"))&&(!checked("nftTerminal")||(isEvmAddress(val("nftContract"))&&mint.ok&&!mint.disabled)))}
...
function applyPayload(p){
  ...
  setValue("openSeaSlug",p.nft?.openSeaSlug);setValue("nftCollectionName",p.nft?.collectionName);setValue("nftSupply",p.nft?.supply);for(const k of ["whaleTracker","memeIntel","nftTerminal","liveMarket"])setValue(k,p.features?.[k]);setNftMintConfiguration(p.nft||{});
  persistedMascot=p.mascot||null; mascotInput.value=""; syncMascotFileName(); refreshBuilderMascotPreview(); update();
}
function resetForm(){form.reset();setValue("version","1.0.0");setValue("promptHost","terminal");setValue("ecosystem","Robinhood Chain");setValue("dexScreenerChainId","robinhood");setValue("blockscoutApiBase","https://robinhoodchain.blockscout.com/api/v2");setValue("nftMintMode","");setValue("nftMintTimezone",browserTimeZone());renderNftPhaseEditor();syncNftMintModeUI();setValue("primary","#39ff14");setValue("accent","#ff6a00");setValue("background","#020806");setValue("panel","#03100b");setValue("whaleTracker",true);setValue("memeIntel",true);setValue("liveMarket",true);activeProjectId="";persistedMascot=null;mascotInput.value="";syncMascotFileName();refreshBuilderMascotPreview();localStorage.removeItem(SETTINGS_KEY);update();status.textContent="[ NEW ] Blank project workspace ready.";loadDeployment()}

...
async function saveProject({duplicate=false}={}){
  ...
  const existing=projects[id];const timestamp=nowIso();projects[id]={id,name:data.projectName.toUpperCase(),ticker:data.ticker,state:(data.projectName&&data.ticker&&data.tokenContract&&(!data.features.nftTerminal||data.nftContract))?"READY":"DRAFT",createdAt:existing?.createdAt||timestamp,updatedAt:timestamp,lastGeneratedAt:existing?.lastGeneratedAt||null,schemaVersion:SCHEMA_VERSION,builderVersion:"1.3.0-B",data};writeProjects(projects);activeProjectId=id;applyPayload(data);refreshProjectList();savedProjectsSelect.value=id;status.textContent=`[ SAVED ] ${data.projectName.toUpperCase()} stored locally.`;
}
...
async function exportProject(){const data=await payload();const bundle={schemaVersion:SCHEMA_VERSION,builderVersion:"1.3.0-B",terminalEngineVersion:"1.0.0",exportedAt:nowIso(),project:data};const blob=new Blob([JSON.stringify(bundle,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`${slugify(data.projectName)||"community-terminal"}-config.json`;a.click();URL.revokeObjectURL(a.href);status.textContent="[ EXPORTED ] Project configuration downloaded."}
...
async function refreshIntegrations(){const label=document.querySelector("#integration-state"),out=document.querySelector("#connected-result"),button=document.querySelector("#connected-deploy");label.textContent="CHECKING";button.disabled=true;try{const response=await fetch("/api/integrations",{cache:"no-store"});const data=await response.json();integrationReady=Boolean(data.enabled&&data.github?.configured&&data.render?.configured);label.textContent=integrationReady?"CONNECTED":data.enabled?"INCOMPLETE":"DISABLED";label.classList.toggle("live",integrationReady);button.disabled=!integrationReady;out.textContent=[`[ ${data.enabled?"OK":"OFF"} ] Connected deployments ${data.enabled?"enabled":"disabled"}`,`[ ${data.github?.configured?"PASS":"WAIT"} ] GitHub server credential`,`[ ${data.render?.configured?"PASS":"WAIT"} ] Render server credential + workspace`,`[ SAFE ] Secrets exposed to browser: ${data.secretsExposed?"YES":"NO"}`].join("\n");}catch(error){label.textContent="UNAVAILABLE";out.textContent=`[ FAIL ] ${error.message}`}}

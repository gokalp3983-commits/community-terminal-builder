...
if (!landingScript.includes('nft: "nftTerminal"')) throw new Error("Landing Page feature filtering is missing");
...
if (!rootServerSource.includes('app.get("/health"') || !rootServerSource.includes('app.get("/status"')) throw new Error("Diagnostic routes missing");
const renderYaml = fs.readFileSync(path.join(generatedRoot, "render.yaml"), "utf8");
if (!renderYaml.includes("healthCheckPath: /health")) throw new Error("Render health check missing");

...
const TOKEN_CONTRACT = process.env.TOKEN_CONTRACT || config.contracts.token;
const DEXSCREENER_CHAIN_ID = config.market.dexScreenerChainId;
...
async function getProjectTokenPrice() {
  if (
    priceCache &&
  ...
  if (!selected) {
    throw new Error(`No liquid ${config.project.name}/WETH market was found`);
  }
  ...
  try {
    const tokenInfo = await fetchJson(
      `${config.market.blockscoutApiBase}/tokens/${TOKEN_CONTRACT}`
    );
...
app.get("/api/price", async (_req, res) => {
  try {
    const market = await getProjectTokenPrice();
    ...
    res.status(502).json({
      error: `Unable to load current ${config.project.name} market data.`,
    });
  }
});
...
const PROJECT_TOKEN_CONTRACT = config.contracts.token;
  ...
  const response = await fetch(
    ...
    `${address}/token-balances`,
    {
      headers: {
        Accept: "application/json",
        "User-Agent":
          `${config.project.id}-nft-terminal/${config.project.version}`,
      },
...
async function fetchSingleProjectTokenBalance(address) {
  const query = new URLSearchParams({
    module: "account",
    action: "tokenbalance",
    contractaddress: PROJECT_TOKEN_CONTRACT,
    address,
    tag: "latest",
  });
  ...
  for (const address of addresses) {
    try {
      ...
      const token = items.find((item) => {
        ...
        return (
          tokenAddress ===
          PROJECT_TOKEN_CONTRACT.toLowerCase()
        );
      });
      ...
      if (token) {
        ...
        balances[address] =
          await fetchSingleProjectTokenBalance(
            address
          );
      }
    } catch (error) {

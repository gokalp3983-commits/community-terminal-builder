...
async function boot() {
  const sequence = [
    [
      ...
      `Initializing ${CFG.project.name} Community Terminal`,
      180,
    ],
    [
      '[ <span class="green">OK</span> ] ' +
...
function setDisconnectedMintStats(){
  ...
  elements.mintedCount.textContent = "— / 4,444";
...
function renderMintStats(stats){
  ...
  elements.mintedCount.textContent =
    `${stats.minted ?? "—"} / ${stats.totalSupply ?? 4444}`;
...
function renderNftSales(data){
  ...
  if(!sales.length){
    ...
    for(const sale of sales){
      ...
      token.textContent = sale.tokenId ? `${CFG.project.name} #${sale.tokenId}` : `${CFG.project.name} NFT`;
...
function formatProjectTokens(value){
  const amount = Number(value);

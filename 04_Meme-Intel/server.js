...
const API_BASE = process.env.BLOCKSCOUT_API_BASE || config.market.blockscoutApiBase;
const TOKEN = process.env.TOKEN_CONTRACT || config.contracts.token;
const DEX_CHAIN = process.env.DEXSCREENER_CHAIN_ID || config.market.dexScreenerChainId;
      ...
      const request = async () => {
        const response = await fetch(url, {
          headers: {
            ...
            "User-Agent": `${config.project.id}-meme-intelligence/${config.project.version}`,
          },
...
async function whalePayload(limit = 100) {
  ...
  return {
    ...
    whales,
    stats: {
      // Canonical concentration fields consumed by the Meme Intel frontend.
...
startActivityBackgroundRefresh();
  console.log("");
  console.log(`🧠 ${config.project.name} Meme Intelligence Terminal`);
  console.log("--------------------------------");
